...
class ReservationValidators:
    ...
    @staticmethod
    def validate_number_of_reservation_per_week(booking_time, name, schedule):
    ...
    @staticmethod
    def validate_hour_is_bookable_for_chosen_day(booking_time, schedule):
        for date, user_reservation in schedule.items():

            # convert date to datetime object, add year
            date_obj = DateTimeConverter.convert_string_date(date, booking_time.year)
            ...
            if date_obj == booking_time.date():
                for user_time_reservation in user_reservation:
                    start_time = DateTimeConverter.convert_string_time(
                        user_time_reservation["start_time"]
                    )
                    end_time = DateTimeConverter.convert_string_time(
